...
python main.py --config "${CONFIG_PATH}" --output_subdir "${WITH_NGRAMS_SUBDIR}" "${EXTRA_ARGS[@]}"

echo "Running with n-grams disabled..."
python main.py --config "${CONFIG_PATH}" --output_subdir "${WITHOUT_NGRAMS_SUBDIR}" --no_ngrams "${EXTRA_ARGS[@]}"
