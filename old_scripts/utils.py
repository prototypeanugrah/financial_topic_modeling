...
def save_model_results(
    ...
    try:
        ...
        # Save topics
        top_topic_words_path = output_dir / f"{prefix}_top_topic_words.txt"
        with open(top_topic_words_path, "w", encoding="utf-8") as f:
            for idx, topic in lda_model.print_topics(-1, num_words=20):
                f.write(f"Topic {idx}: {topic}\n")

        all_topic_words_path = output_dir / f"{prefix}_all_topic_words.txt"
        all_topic_words = lda_model.get_topics()
        id2word = lda_model.id2word
        with open(all_topic_words_path, "w", encoding="utf-8") as f:
            for topic_idx, topic_probs in enumerate(all_topic_words):
                sorted_word_ids = topic_probs.argsort()[::-1]  # sort by prob desc
                for word_id in sorted_word_ids:
                    prob = float(topic_probs[word_id])
                    if id2word is not None:
                        ...
                        word = str(word_id)
                    f.write(f"Topic {topic_idx}\t{word}\t{prob:.6f}\n")

        # Save metrics
        metrics_path = output_dir / f"{prefix}_eval_metrics.txt"
...
def plot_perplexity_scores(
    ...
    try:
        ...
        plot_path = output_dir / f"{prefix}_perplexity_plot_{mode}.png"
...
def save_topic_perplexity_scores(
    ...
    try:
        ...
        scores_path = output_dir / f"{prefix}_topic_perplexity_scores_{mode}.txt"
...
def analyze_word_frequencies(
    ...
    try:
        ...
        # Save plot
        plot_path = output_dir / f"{prefix}_word_frequency_plot.png"
        plt.savefig(plot_path, dpi=300, bbox_inches="tight")
        plt.close()
